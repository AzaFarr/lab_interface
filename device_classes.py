...
class Tensiometer():
    ...
    def __init__(self, notebook):
        self.TENZ = ttk.Frame(master=notebook, borderwidth=3, relief=RAISED)

        self.TENZ.columnconfigure(index=0, weight=1)
        self.TENZ.columnconfigure(index=1, weight=0)
        self.TENZ.columnconfigure(index=2, weight=50)

        self.TENZ.rowconfigure(index=0, weight=0)
        self.TENZ.rowconfigure(index=1, weight=1)
        self.TENZ.rowconfigure(index=2, weight=1)
        self.TENZ.rowconfigure(index=3, weight=10)

        self.data_table = []

    def On(self):
        label_2 = ttk.Label(master=self.TENZ, borderwidth=0, font=("Arial", 13), justify=LEFT, text="Сведения об эксперименте")
        label_2.grid(column=0, row=0, columnspan=2, sticky=NW, padx=5, pady=5)

        table_frame = Frame(master=self.TENZ)
        ...
        label_1 = ttk.Label(master=self.TENZ, borderwidth=0, font=("Arial", 20), justify=LEFT, text="Показание с датчика")
        label_1.grid(column=1, row=1, sticky=NW, padx=30, pady=0)

        #TODO: make this entry window like display to show values
        enter_value = ttk.Entry(master=self.TENZ, font=("Arial", 40), justify=LEFT, width=8)
        enter_value.grid(column=1, row=1, sticky=NW, padx=30, pady=40)

        button_1 = Button(master=self.TENZ, text='Добавить')
        root_button_1 = ButtonMy(button_1, self.TENZ)
        ...
        button_2 = Button(master=self.TENZ, text='Авто-ввод')
        root_button_2 = ButtonMy(button_2, self.TENZ)
